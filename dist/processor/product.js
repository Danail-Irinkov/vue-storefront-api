...
class ProductProcessor {
    ...
    process(items, groupId = null) {
        ...
        const factory = new factory_1.default(this._config, this._req);
        ...
        return Promise.all(processorChain).then(((resultSet) => {
            ...
            if (this._req.query._source_exclude && this._req.query._source_exclude.indexOf('sgn') < 0) {
                const rs = resultSet[0].map(((item) => {
                    ...
                    item._source.sgn = hmac.sign(util_1.sgnSrc(sgnObj, item), config.objHashSecret); // for products we sign off only price and id becase only such data is getting back with orders
                    if (item._source.configurable_children) {
                        item._source.configurable_children = item._source.configurable_children.map((subItem) => {
                            if (subItem) {
                                ...
                                subItem.sgn = hmac.sign(util_1.sgnSrc(sgnObj, subItem), config.objHashSecret);
                            }
                            return subItem;
                        });
                    }
