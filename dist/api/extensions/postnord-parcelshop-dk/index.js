...
module.exports = ({ config, db }) => {
    let glsApi = express_1.Router();
    glsApi.get('/zipcode/:zipcode', (req, res) => {
        if (req.params.zipcode.length != 4) {
            return util_1.apiStatus(res, { error: 'Invalild zipcode length.' }, 200);
        }
        let returnData = {};
        ...
        request({
            ...
            if (_resBody && _resBody.servicePointInformationResponse.servicePoints) {
                ...
                return util_1.apiStatus(res, returnData, 200);
            }
            return util_1.apiStatus(res, { error: 'Invalid zipcode.' }, 200);
        });
    });
    return glsApi;
};
