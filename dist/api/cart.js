...
exports.default = ({ config, db }) => {
    ...
    cartApi.post('/update', (req, res) => {
        ...
        if (!req.body.cartItem) {
            return util_1.apiStatus(res, 'No cartItem element provided within the request body', 500);
        }
    ...
    cartApi.post('/apply-coupon', (req, res) => {
        const cartProxy = _getProxy(req);
        if (!req.query.coupon) {
            return util_1.apiStatus(res, 'No coupon code provided', 500);
        }
    ...
    cartApi.post('/shipping-methods', (req, res) => {
        ...
        res.setHeader('Cache-Control', 'no-cache, no-store');
        if (!req.body.address) {
            return util_1.apiStatus(res, 'No address element provided within the request body', 500);
        }
    ...
    cartApi.post('/collect-totals', (req, res) => {
        ...
        res.setHeader('Cache-Control', 'no-cache, no-store');
        if (!req.body.methods) {
            return util_1.apiStatus(res, 'No shipping and payment methods element provided within the request body', 500);
        }
