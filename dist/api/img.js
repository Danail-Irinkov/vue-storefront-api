...
exports.default = ({ config, db }) => asyncMiddleware((req, res, body) => tslib_1.__awaiter(this, void 0, void 0, function* () {
    if (!(req.method == 'GET')) {
        res.set('Allow', 'GET');
    ...
    const mimeType = mime_types_1.default.lookup(imgUrl);
    ...
    let buffer;
    try {
        buffer = yield image_1.downloadImage(imgUrl);
    }
    catch (err) {
        return res.status(400).send({
            code: 400,
    ...
    switch (action) {
        case 'resize':
            return res
                ...
                .send(yield image_1.resize(buffer, width, height));
        case 'fit':
            return res
                .type(mimeType)
                .set({ 'Cache-Control': `max-age=${ONE_YEAR}` })
                .send(yield image_1.fit(buffer, width, height));
        case 'identify':
            return res.set({ 'Cache-Control': `max-age=${ONE_YEAR}` }).send(yield image_1.identify(buffer));
        default:
